...
void DXUTParseCommandLine( WCHAR* strCommandLine )
{
    ...
    for( int iArg=1; iArg<nNumArgs; iArg++ )
    {
        ...
        if( *strCmdLine == L'/' || *strCmdLine == L'-' )
        {
            ...
            if( DXUTIsNextArg( strCmdLine, L"nvperfhud" ) )
            {
                g_bCreateNVDevice = true;
                continue;
            }
...
HRESULT WINAPI DXUTCreateDevice( bool bWindowed, int nSuggestedWidth, int nSuggestedHeight )
{
    ...
    // FORCING no vsync
    if( 1 ) //GetDXUTState().GetOverrideForceVsync() == 0 )
    {
...
HRESULT DXUTCreate3DEnvironment9( IDirect3DDevice9* pd3dDeviceFromApp )
{
    ...
    if( pd3dDeviceFromApp == NULL )
    {
        ...
        if(g_bCreateNVDevice)
        {
            ...
            pNewDeviceSettings->d3d9.AdapterOrdinal = AdapterToUse;
            pNewDeviceSettings->d3d9.DeviceType = DeviceType;
            pNewDeviceSettings->d3d9.BehaviorFlags = D3DCREATE_HARDWARE_VERTEXPROCESSING;
        }
        hr = pD3D->CreateDevice( pNewDeviceSettings->d3d9.AdapterOrdinal, pNewDeviceSettings->d3d9.DeviceType, 
            DXUTGetHWNDFocus(), pNewDeviceSettings->d3d9.BehaviorFlags,&pNewDeviceSettings->d3d9.pp, &pd3dDevice );
