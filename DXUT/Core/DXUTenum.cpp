...
HRESULT CD3D10Enumeration::EnumerateDevices( CD3D10EnumAdapterInfo *pAdapterInfo )
{
    ...
    for( UINT iDeviceType = 0; iDeviceType < devTypeArrayCount; iDeviceType++ )
    {
        ...
        hr = DXUT_Dynamic_D3D10CreateDevice( pAdapter, devTypeArray[iDeviceType], (HMODULE)0, 0, NULL, D3D10_SDK_VERSION, &pd3dDevice );
        if( FAILED( hr ) )
        {
            delete pDeviceInfo;
            continue;
...
void CD3D10Enumeration::BuildMultiSampleQualityList( DXGI_FORMAT fmt, CD3D10EnumDeviceSettingsCombo* pDeviceCombo )
{
    ...
    if( FAILED( DXUT_Dynamic_D3D10CreateDevice( pAdapter, pDeviceCombo->DeviceType, (HMODULE)0, 0, NULL, D3D10_SDK_VERSION, &pd3dDevice ) ) )
        return;
