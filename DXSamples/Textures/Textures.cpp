...
DWORD WINAPI RenderThreadProc(LPVOID pParam)
{
    while(true)
    {
        WaitForSingleObject(g_RenderRecordSem, INFINITE);

        // Swap in recording device
        LPDIRECT3DDEVICE9 pd3dDevice = g_pd3dDevice;
        g_pd3dDevice = g_pCBRecorder;

        g_pCBRecorder->BeginCommandBuffer();
        Render();
        g_pCBRecorder->EndCommandBuffer();

        // Restore real device
        g_pd3dDevice = pd3dDevice;

        ReleaseSemaphore(g_RenderPlaybackSem, 1, NULL);
    }
    return 0;
}
...
{
    LONG previous;
    ReleaseSemaphore(g_RenderRecordSem, 1, &previous);

    WaitForSingleObject(g_RenderPlaybackSem, INFINITE);

    g_pCBPlayer->Playback(g_pd3dDevice, &g_CB);
}

void InitThreads()
{
    g_RenderRecordSem = CreateSemaphore(NULL, 0, 1, NULL);
    g_RenderPlaybackSem = CreateSemaphore(NULL, 0, 1, NULL);
    ...
    if( SUCCEEDED( InitD3D( hWnd ) ) )
    {
        ...
        if( SUCCEEDED( InitGeometry() ) )
        {
            ...
            while( msg.message!=WM_QUIT )
            {
                ...
                {
                    // Instead of just calling render, we call Playback
                    // which requests a render from another thread, then 
                    // plays back the commands.
