...
void InitApp()
{
    ...
    StringCchPrintf( szMessage, 300, L"# Boxes: %d", g_NumBoxes ); 
    ...
    iY += 24;
    StringCchCopy( szMessage, 300, L"Goal: 0ms/frame" );
    g_SampleUI.AddStatic( IDC_BIND_CPU_STATIC, szMessage, 0, iY += 12, 200, 22 );
    StringCchCopy( szMessage, 300, L"Remaining for logic: 0" );
    ...
    StringCchCopy( szMessage, 300, L"If rendering takes less\n"
...
HRESULT OnCreateBuffers( IDirect3DDevice9* pd3dDevice )
{
    ...
    if( 1 == g_iRenderTechnique )
    {
        ...
        V_RETURN( pd3dDevice->CreateVertexBuffer( g_nNumBatchInstance * 4 * 6 * sizeof( BOX_VERTEX_INSTANCE ), 0, 0, D3DPOOL_MANAGED, & g_pVBBox, 0 ) );
        ...
        // 6 * 2 * 3 indices
        V_RETURN( pd3dDevice->CreateIndexBuffer( g_nNumBatchInstance * ( 6 * 2 * 3 ) * sizeof( WORD ), 0, D3DFMT_INDEX16, D3DPOOL_MANAGED, &g_pIBBox, 0 ) );
    ...
    {
        // Build a VB to hold the position data. Four vertices on six faces of the box
        V_RETURN( pd3dDevice->CreateVertexBuffer( 4 * 6 * sizeof( BOX_VERTEX ), 0, 0, D3DPOOL_MANAGED, & g_pVBBox, 0 ) );

        // And an IB to go with it. We will be rendering
        // a list of independent tris, and each box face has two tris, so the box will have
        // 6 * 2 * 3 indices
        V_RETURN( pd3dDevice->CreateIndexBuffer( ( 6 * 2 * 3 ) * sizeof( WORD ), 0, D3DFMT_INDEX16, D3DPOOL_MANAGED, &g_pIBBox, 0 ) );
    ...
    if( 0 == g_iRenderTechnique || 3 == g_iRenderTechnique )
    {
        ...
        V_RETURN( pd3dDevice->CreateVertexBuffer( g_NumBoxes * sizeof( BOX_INSTANCEDATA_POS ), 0, 0, D3DPOOL_MANAGED, &g_pVBInstanceData, 0 ) );
            ...
            for( BYTE iY = 0; iY < 10; iY++ )
                for( BYTE iZ = 0; iZ < 10; iZ++ )
                    for( BYTE iX = 0; iX < 10 && nRemainingBoxes > 0; iX++, nRemainingBoxes-- )
                    {
                        ...
                        instanceBox.color = D3DCOLOR_ARGB( 0xff, 0x7f + 0x40 * ((g_NumBoxes - nRemainingBoxes + iX) % 3), 0x7f + 0x40 * ((g_NumBoxes - nRemainingBoxes+iZ+1) % 3), 0x7f + 0x40 * ((g_NumBoxes - nRemainingBoxes+iY+2) % 3) );
                        instanceBox.x = iZ * 24;
                        instanceBox.z = iX * 24;
                        instanceBox.y = iY * 24;
        ...
        for( BYTE iY = 0; iY < 10; iY++ )
            for( BYTE iZ = 0; iZ < 10; iZ++ )
                for( BYTE iX = 0; iX < 10 && nRemainingBoxes > 0; iX++, nRemainingBoxes-- )
                {
                    ...
                    // The positins are scaled and biased.
                    // The rotations are not
                    g_vBoxInstance_Color[g_NumBoxes - nRemainingBoxes] = D3DCOLOR_ARGB( 0xff, 0x7f + 0x40 * ((g_NumBoxes - nRemainingBoxes + iX) % 3), 0x7f + 0x40 * ((g_NumBoxes - nRemainingBoxes+iZ+1) % 3), 0x7f + 0x40 * ((g_NumBoxes - nRemainingBoxes+iY+2) % 3) );
                    ...
                    g_vBoxInstance_Position[g_NumBoxes - nRemainingBoxes].w = ((WORD)iX + (WORD)iZ + (WORD)iY) * 8 / 255.0f;
                }
    }   
...
void CALLBACK OnFrameMove( double fTime, float fElapsedTime, void* pUserContext )
{
    ...
    while( fCurTime < fStopTime )
    {
        ...
        StringCchPrintf( szTmp, 100, L"Test %d", (int)(fTmp + 0.5f) ); 
    }
    ...
    g_fLastTime = DXUTGetGlobalTimer()->GetTime();
    StringCchPrintf( szMessage, 100, L"Remaining for logic: %d%%", (int)(fCPUUsage + 0.5f) ); szMessage[99] = 0;
...
#if 1
    RedirectingEffectStateManager Manager( pd3dDevice );
    RedirectingEffectStateManager *pManager = &Manager;

    V( g_pEffect->GetStateManager(&pOldManager) );
    V( g_pEffect->SetStateManager(pManager) );
#endif
...
#if 1
    V( g_pEffect->SetStateManager(pOldManager) );
#endif
        
    g_pRecorder->EndCommandBuffer();  
    //pD3DDev9->RestoreCurrentDevice();
    //g_pPlayer->Playback(pD3DDev9->GetRealDevice(),g_pMBuffer);
    
    g_pPlayer->Playback(pd3dSavedDevice,g_pMBuffer);
    //pD3DDev9->Release();
    //OutputDebugStringA("Done Playing\n");
...
void OnRenderStreamInstancing( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
    ...
    for( iPass = 0; iPass < cPasses; iPass++ )
    {
        ...
        {   
            ...
            V( pd3dDevice->SetStreamSource( 1, g_pVBInstanceData, nRemainingBoxes * sizeof( BOX_INSTANCEDATA_POS ), 0 ) );
...
void CALLBACK KeyboardProc( UINT nChar, bool bKeyDown, bool bAltDown, void* pUserContext )
{
    if( bKeyDown )
    {
        switch( nChar )
        {
            ...
            case 'U': g_nShowUI--; if( g_nShowUI < 0 ) g_nShowUI = 2;            
        }
    }
}
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl, void* pUserContext )
{
    ...
    switch( nControlID )
    {
        ...
        case IDC_NUMBERBOXES_SLIDER:
            ...
            StringCchPrintf( szMessage, 100, L"# Boxes: %d", g_NumBoxes ); szMessage[99] = 0;
        ...
        case IDC_BIND_CPU_SLIDER:
             ...
             StringCchPrintf( szMessage, 100, L"Goal: %0.1fms/frame", g_fBurnAmount * 1000.0f ); szMessage[99] = 0;
